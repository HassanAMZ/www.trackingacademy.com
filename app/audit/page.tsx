    ...
    <Container className="space-y-8 py-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Comprehensive analysis of your website's tracking and analytics implementation
        </p>
      </div>
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {audits.map((audit) => (
          <Card key={audit.id} className="shadow-md transition-shadow hover:shadow-lg">
            <CardHeader className="pb-2">
              <div className="flex items-start justify-between">
                ...
                <Badge className={getTextColor(audit.statusColor)} variant="outline">
                  {audit.status}
                </Badge>
              </div>
            </CardHeader>
            <CardContent>
              <div className="mb-4">
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <span className={`text-lg font-bold ${getTextColor(audit.statusColor)}`}>
                    {audit.score}/100
                  </span>
                </div>
                <Progress
              ...
              <div className="space-y-2">
                ...
                {audit.categories.map((category, index) => (
                  <div key={index} className="flex items-center justify-between text-sm">
                    ...
                    <span className={getTextColor(category.color)}>{category.score}/100</span>
                  </div>
                ))}
              </div>
            </CardContent>
