...
export async function POST(req) {
  try {
    ...
    const { priceId, promotionCode } = body; // Get the Stripe price to retrieve the amount
    ...
    let promoDetails = null; // Add promotion code if provided
    if (promotionCode) {
      ...
      if (promotionCodes.data.length > 0) {
        const validPromo = promotionCodes.data[0]; // Get the coupon details
        const coupon = await stripe.coupons.retrieve(validPromo.coupon.id); // Calculate discount
