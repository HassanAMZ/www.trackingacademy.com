...
export async function POST(req: Request) {
  ...
  const OfferSchema = z.object({
    mainOffer: z.object({
      headline: z
        .string()
        .describe('Attention-grabbing headline that captures the core value proposition'),
      dreamOutcome: z.string().describe('Detailed description of the ultimate result'),
      targetAudience: z.string().describe('Specific audience most likely to benefit'),
    }),
    uniqueProcess: z.object({
      mechanism: z.string().describe('Unique approach or method that sets this offer apart'),
      stepByStepBreakdown: z.array(z.string()).describe('Detailed steps of the process'),
    }),
    valueProposition: z.object({
      effortRequired: z.string().describe('Detailed explanation of effort and commitment'),
      timeCommitment: z.string().describe('Expected time investment'),
      successProbability: z.string().describe('Likelihood of achieving the desired outcome'),
    }),
    risksAndGuarantees: z.object({
      potentialChallenges: z
        .array(z.string())
        .describe("Potential obstacles and how they're addressed"),
      guarantee: z.string().describe('Specific guarantee or risk reversal mechanism'),
      bonus: z.string().describe('Additional bonus to increase perceived value'),
    }),
