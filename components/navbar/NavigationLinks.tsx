    ...
    if (link.children && link.children.length > 0) {
     return (
      <div key={`nav-link-${link.title}`} className='group relative'>
       ...
       <div className='nested-links border-2 border-dominant absolute hidden group-hover:flex flex-row bg-complementary shadow-md p-2 rounded-lg z-20'>
        {link.children.map((childLink) => (
         <NavLink
          key={`nav-link-${childLink.title}`}
          href={childLink.href}
          className='hover:underline hover:text-accent transition-all duration-300 mx-2'>
          {childLink.title}
         </NavLink>
        ))}
       </div>
